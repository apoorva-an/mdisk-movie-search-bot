...
async def start_handler(_, m: Message):
    await m.reply_text("Hi, I am Alive!\n\nSearch using /request command to Search Movies And Series.", quote=True)
...
async def text_handler(_, m: Message):
    ...
    editable = await m.reply_text("Please Wait Iam Searching For Your Request Please Wait...", quote=True)
    response = await search_pdisk_videos(m.text.split(" ", 1)[-1], Configs.PDISK_USERNAME, Configs.PDISK_PASSWORD)
    if isinstance(response, Exception):
        traceback.print_exc()
        try: await editable.edit("Failed to searching!",
                                 reply_markup=InlineKeyboardMarkup([
                                     [InlineKeyboardButton("Sumpot Group", url="https://t.me/Updates_kannada_channels")]
                                 ]))
